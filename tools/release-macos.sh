...
- 動画のサムネイルは mp4 と mov で出ることを実測済みです（H.264 とHEVCの見本）。
  .avi は枠だけになることを実測済みで、他のコンテナは未測定です。
  Windows版はエクスプローラと同じ経路なので、もう少し広く借りられます
...
macOS版は**そのまま表示できます**。Windows版と違い、入れるものはありません
