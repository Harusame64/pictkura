...
このダイアログには先へ進むボタンがありません。［完了］を押してから、
次の手順で開いてください。

  1. pictkura.app を「アプリケーション」フォルダなど、置いておきたい場所へ移す
  2. ダブルクリックする（上のメッセージが出るので［完了］）
  3. システム設定 →「プライバシーとセキュリティ」を開き、下の方までスクロールする
  4. pictkura がブロックされた旨の行にある「このまま開く」を押す
  5. パスワードを求められたら入れる。アプリが起動します

初回だけです。2回目からは普通にダブルクリックで開きます。

ターミナルの方が早ければ、次の1行でも同じことができます。

  xattr -dr com.apple.quarantine /パス/pictkura.app

※ 右クリック（controlキーを押しながらクリック）→「開く」は効きません。
   macOS 14 まではこれで開けましたが、macOS 15 以降この抜け道は廃止されました。
   古い手順を書いた記事がまだ多く残っているのでご注意ください。

※ 手順1で先に移しておくのは作法ではありません。移さずに起動すると、macOSは
   アプリを読み取り専用の一時領域へ写して実行します（App Translocation）。
