...
- 使い方: アプリの設定 →「このアプリについて」→「取扱説明書」で開けます
- ライセンス: LICENSE（MIT）
...
■ Windows版のほうが広い範囲をまかなうところ

- 動画のサムネイルは mp4 と mov で出ることを実測済みです（H.264 と、
  HEVC〈hvc1＝iPhoneと同じ綴り〉の見本で。ただし見本は macOS 自身で変換したもので、
  携帯電話が書いた実物ではありません）。.avi は枠だけになることを実測済みで、
  他のコンテナは未測定です。Windows版はエクスプローラと同じ経路なので、
  もう少し広く借りられます
- mp4 / m4v / mov / 3gp 以外の動画の長さと大きさは出ません。
  Windows版はここをシェルに聞きますが、macOS版にその後ろ盾がありません
- クラウドにしか実体が無いファイルの撮影日などは、macOS版では埋まりません

■ HEIC/HEIF について

macOS版はそのまま表示できます。Windows版と違い、入れるものはありません
（ImageIOもHEVCのデコーダもOSに同梱されています）。
EOF
