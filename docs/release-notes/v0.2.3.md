...
nothing for it either. Containers other than those three have not been measured either
way.
...
network share that stopped answering mid-session is the case this was measured against,
with the connection still up — is now
...
Spotlight も何も返しません。この3つ以外のコンテナは、出るとも出ないとも
**測っていません**。
...
AVFoundation から借りており、`mp4` と `mov` は H.264 と HEVC（`hvc1`）の見本で出ること、`.avi` は何も
