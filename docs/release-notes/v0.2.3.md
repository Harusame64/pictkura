...
system's own decoder, on all five paths that can reach a HEIF (photo, embedded thumbnail,
viewer, and the two RAW previews that are stored as HEVC). The picture is drawn into an
sRGB bitmap so CoreGraphics does the colour conversion, which is what the Windows path has
always done, so both platforms come out of the decoder in the same format.

Measured on Apple Silicon with real 24MP iPhone HEICs and public CC0 samples: a main image
takes 330–357ms where before there was no decoder, and an embedded thumbnail about 15ms.
RAW coverage on the sample set went from 9 of 10 to 10 of 10.

**Videos get thumbnails on macOS.** They were all empty frames. pictkura now pulls the
first frame through AVFoundation, so `mp4` and `mov` show a picture — measured at 80–92ms
for the first tile in a grid and 9.6–10.4ms for every one after it, since only the first
pays AVFoundation's start-up cost. Rotation is handled by the framework, so portrait clips
from a phone come out the right way up.
...
nothing for it either. Containers other than `mp4` and `mov` have not been measured either
way.
...
unplugged; the folder holds a Photos, iPhoto or Aperture library, with wording that
matches which one, since only the Photos library keeps most originals in iCloud;
everything found was excluded; or the folder genuinely has no photos in it.

It also stops speaking with more confidence than it has. A folder that never answers — a
network share that went away mid-session is the case this was measured against — is now
reported as exactly that, with the place named, rather than as "still looking". Each
...
working with H.264 samples and `.avi` measured returning nothing. The frame taken is always
...
**ImageIO** で展開します。HEIF に行き着く5つの入口（写真・埋め込みサムネイル・ビューア・
HEVC で入っている RAW のプレビュー2種）すべてに入れてあります。取り出した絵はいったん
sRGB のビットマップに描き直して CoreGraphics に色を変換させます——Windows 側が前から
やっていることと同じで、両方のOSでデコーダの出口が揃います。

Apple Silicon で、実物の 24MP の iPhone HEIC と CC0 の見本で測りました。写真そのものは
**330〜357ミリ秒**（それまでは「デコーダが無い」でした）、埋め込みサムネイルは15ミリ秒
ほどです。見本の RAW は10件中9件だったのが**10件中10件**になりました。

**macOS で動画にサムネイルが出ます。** これまでは全部が枠だけでした。いまは
**AVFoundation** で先頭のコマを取り出すので、`mp4` と `mov` に絵が出ます。一覧の
**1枚目が80〜92ミリ秒、2枚目以降は9.6〜10.4ミリ秒**——AVFoundation の初回読み込みを
払うのが最初の1枚だけだからです。向きは AVFoundation 側が直すので、スマートフォンの
縦位置の動画もそのまま正しく出ます。

**`.avi` は macOS では枠だけのままです。** しかもこれは、どの道からも届かないことが
分かっている唯一のコンテナです——QuickLook は45秒返らず（強制終了しました）、
Spotlight も何も返しません。`mp4` と `mov` 以外のコンテナは、出るとも出ないとも
**測っていません**。
...
（名前を出します。たいていは繋いでいない外付けドライブです）／中身が写真アプリ・iPhoto・
Aperture のライブラリである（**どれに当たったかで文面を変えます**。原本がiCloudにあるのは
写真アプリだけなので）／見つかったものが全部除外された／本当に写真が1枚も無い。

**分かっていないことを分かったように言うのもやめました。** 返事をしないフォルダ
——途中で消えたネットワーク共有で実測しました——は「まだ探しています」ではなく、
...
AVFoundation から借りており、`mp4` と `mov` は H.264 の見本で出ること、`.avi` は何も
返らないことを実測しています。取り出すコマは常に先頭なので、黒からフェードインする動画は
