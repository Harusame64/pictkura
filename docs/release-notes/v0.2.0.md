...
without ever waiting for an image to appear.
...
that is already there. Until the full image arrives, the thumbnail is laid underneath, so
you always see the photo rather than a blank. Double-click (or `1`) toggles 100%, `0`
fits to the window.
...
shots come out upright, and older bodies that only carry a small preview are documented
in the README rather than failing silently.
...
nothing else. Turn it off in ⚙ → About pictkura.

**It does not fall over on a broken file.** Panics were removed from the parsing and
thumbnail paths; a corrupt or truncated file is skipped instead of taking the app with it.
...
ボツを外し、最後にまとめてゴミ箱へ——その間、絵が出るのを待たされません。
...
`←` `→` は既に用意できている絵に切り替わるだけなので、待ち時間がありません。原寸が
届くまではサムネイルを下に敷くので、真っ白ではなく写真が見えます。ダブルクリック
...
**壊れた写真1枚でアプリが落ちません。** 解析とサムネイル生成の経路からパニックを
無くしました。壊れたファイルや途中で切れたファイルは、アプリを道連れにせず飛ばします。
