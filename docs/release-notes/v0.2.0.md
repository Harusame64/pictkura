...
JPEG before showing it. That step now goes through mozjpeg: opening a 24.5 MP HEIC
full-screen dropped from about 1.0 s to about 0.6 s (measured, same machine).
...
pictkura 0.2.0 は**選別**のリリースです。連写をビューアで送りながら残す1枚に印を
...
JPEGへ詰め直しています。その処理を mozjpeg に替え、**24.5MPのHEICを全画面で開くまでが
約1.0秒から約0.6秒**になりました（同じ機械での実測）。
