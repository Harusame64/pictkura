...
holding. The pair of old Panasonic `.RAW` files in the sample set (DMC-LX1, DMC-FZ8) has
no preview and no declared size, but does carry a maker, a model and a capture time; both
were filed under the day they were copied and had dropped out of the camera filter. They
now read 2018-06-19 and 2018-01-26, and both cameras register.

The size is still the only thing given up, and only when nothing declares it — writing a
zero there would erase what the operating system had lent us.
...
still be corrected by its name, as before. This hole was not specific to RAW: every file
that gets looked at more than once could hit it.
...
and `.avi` appear in the grid but do not play inside the app. There is still no sort order
...
まさにこれで、プレビューも寸法の申告も無い代わりに、メーカー名・機種名・撮影日時は
持っています。2枚ともコピーした日のフォルダに並び、カメラの絞り込みからも
消えていました。いまは 2018-06-19 と 2018-01-26 と読め、カメラも登録されます。

諦めるのは寸法だけで、しかも**どこにも申告が無いときだけ**です——ここに 0 を書くと、
OSが貸してくれていた値を消してしまうためです。
...
これまでどおりファイル名で直せます。この穴は RAW に限った話ではなく、
**二度以上調べ直されるファイルすべて**にありました。
