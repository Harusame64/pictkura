...
inside them: they stop landing on the day you copied them, and the ones that do write
their size — somewhere pictkura was not looking — finally get one.
...
No sweep was added to go looking for photos to re-measure. The pass that already fills in
sizes after the fact takes only rows with no preview size recorded, which any photo
imported by 0.2.2 or later already has — a library older than that gets its `.rwl`
corrected by that pass instead.
...
コピーした日のフォルダに並んでしまうのをやめ、**寸法を書いてはいたもの**
...
**探して回る処理を新しく足してはいません。** もともとある
「あとから寸法を埋める掃き」が見るのは「プレビューの寸法が記録されていない行」
だけで、0.2.2 以降に取り込んだ写真は
