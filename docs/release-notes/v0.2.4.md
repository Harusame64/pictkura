...
still be corrected by its name, as before. This is the chain taken by a file no picture
could be made for, whether or not it declares a size, and that is the path where the
repeated looks happen. A file whose thumbnail was made goes through a different chain,
and so do videos; this release does not touch either, so a photo that is processed again
later — a rescan, or the thumbnail cache rebuilding its tile — can still have a good date
replaced this way.
...
これまでどおりファイル名で直せます。この連鎖を通るのは**絵を作れなかったファイル**で、
寸法を申告するかどうかは問いません——調べ直しが何度も起きるのがこの道だからです。
**サムネイルが作れたファイルは別の連鎖**を通り、動画もまた別です。どちらも
**このリリースでは触っていません**ので、あとから作り直される写真——再スキャンや、
サムネイルの置き場から追い出されて作り直されたとき——は、いまも同じ形で
日付を書き換えられることがあります。
