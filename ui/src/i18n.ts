...
const ja = {
  ...
  moveConfirm: (n: number) =>
    n === 1
      ? "この写真を選んだフォルダへ移動しますか？ 元の場所からは無くなり、ライブラリからも外れます（お気に入りは引き継がれません）。"
      : `${n}枚の写真を選んだフォルダへ移動しますか？ 元の場所からは無くなり、ライブラリからも外れます（お気に入りは引き継がれません）。`,
...
const en: Dict = {
  ...
  moveConfirm: (n: number) =>
    n === 1
      ? "Move this photo to the folder you picked? It leaves its current place and leaves the library (favorites are not carried over)."
      : `Move ${n} photos to the folder you picked? They leave their current place and leave the library (favorites are not carried over).`,
  exporting: (done: number, total: number, name: string) =>
