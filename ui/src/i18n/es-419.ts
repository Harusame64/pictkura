 ...
 * ## なぜ全文を書き写さないのか
 *
 * **2つのスペイン語が食い違うのは、下で上書きしている鍵だけ**である。残りは1文字も違わない。
 * **数は書かない**——鍵を足すたびに古びる（2026-09-25 に「21」が 22 のまま残っていた）。数えるなら
 * `export const es419` の中の `  鍵:` の行を数える。
 ...
 * **大小を無視して、語の頭から**2方向から使う（語の中ほどは見ない——`dañado` の
 * 中の `añad` を拾って落ちたことがある）:
 *
