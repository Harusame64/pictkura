 ...
 *
 * **辞書にアラビア語が無いから届かない、とも読まないこと。** `formatLocale` は
 * **辞書の言語＋OSの地域**で組み直すので、エジプトの人が英語の辞書へ落ちても
 * `en-EG` になり、週の始まりは地域副タグが決める（`en-EG` / `ja-EG` とも
 * `firstDay=6`）。**土曜始まりは公開中の版が既に描いている画面**である。
 * 言語で括る見分け方も使えない——`ar-SA` は日曜、`ar-AE` は月曜。
 */
