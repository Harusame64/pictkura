...
export const zhHant: Dict = {
  ...
  shortcutGroups: [
    ...
    },
    {
      ...
      keys: [
        ...
        ["空白鍵", "投影片放映。影片則是播放 / 暫停"],
        ["1 / 0", "1 在實際大小 100% 與符合視窗之間切換；0 一律符合視窗"],
        ["F11", "全螢幕"],
        ["Esc", "關閉"],
      ],
    },
    {
      ...
      keys: [
        ...
        ["滾輪", "放大 / 縮小"],
        ["拖曳", "放大後移動畫面"],
        ["按右鍵", "開啟 / 用其他程式開啟… / 在資料夾中顯示 / 加入最愛 / 留用 / 刪除（移到資源回收筒）"],
        ["按一下下方的底片條", "跳到那張照片"],
      ],
    },
  ...
  drivesReturnedScanned: (added: number, changed: number) =>
    `已重新讀取插入的磁碟機上的資料夾（新增 ${num(added)}，變更 ${num(changed)}）`,
  ...
  moveDone: (moved: number, skipped: number, failed: number, leftBehind: number) => {
    const parts: string[] = [];
    if (moved > 0 || skipped + failed + leftBehind === 0) parts.push(`已移動 ${num(moved)} 張`);
    if (skipped > 0) parts.push(`有 ${num(skipped)} 張在目標位置已經存在，留在了原來的位置`);
    if (failed > 0) parts.push(`有 ${num(failed)} 張無法移動，仍在原來的位置`);
