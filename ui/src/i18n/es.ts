...
export const es: Dict = {
  ...
  shortcutGroups: [
    ...
    },
    {
      ...
      keys: [
        ...
        ["Clic derecho", "Abrir / Abrir con otra aplicación… / Mostrar en la carpeta / Añadir a Favoritos / Marcar con un indicador / Borrar (mover a la papelera)"],
  ...
  rootTempConfirm: (path: string) =>
    `«${path}» está dentro de una carpeta temporal.\nEl sistema u otras aplicaciones pueden borrar sin avisar los archivos que contiene. ¿Añadirla a la biblioteca de todos modos?`,
  rootTempConfirmOk: "Añadir de todos modos",
  destTempConfirm: (path: string) =>
    `«${path}» está dentro de una carpeta temporal.\nEl sistema u otras aplicaciones pueden borrar sin avisar las fotos que importes aquí. Cuando borres la tarjeta, no quedará ninguna otra copia. ¿Usarla como destino de todos modos?`,
  ...
  drivesReturnedScanned: (added: number, changed: number) =>
    `Se han vuelto a leer las carpetas de la unidad conectada (${num(added)} ${one(added, "añadida", "añadidas")}, ${num(changed)} ${one(changed, "cambiada", "cambiadas")})`,
  ...
  emptyStartupFailed:
    "La sincronización que se ejecuta al arrancar no terminó. Puede que haya fotos que todavía no se hayan incorporado a la biblioteca. Pulsa «Volver a explorar»; si no sirve, vuelve a abrir la aplicación.",
