...
export interface MediaItem {
  ...
  /** `taken_at_ms` が秒未満まで分かっているか（連写の条件。dev #32） */
  taken_subsec: boolean;
  /** 機体の鍵（機種＋本体シリアル。0 は分からない。dev #32） */
  body_key: number;
}
...
export interface EmptyLibraryReason {
  ...
  missing: string[];
  /** 実在するのに**読めなかった**場所（権限・macOSのTCC・切れたネットワーク）。
   *  ルート自身に加えて、**走査が奥で開けなかったフォルダ**も入る（先頭3件まで） */
  unreadable: string[];
  /** 開けなかった場所の数（3件で切ったぶんを「ほか N件」と言うため）。
   *  **名前の数では足りない**——走査は先頭3件しか控えていない */
  ...
  /** **まだ確かめ終わっていない**（探りが返らないまま時間切れ）。
   *  刺さったネットワークのフォルダで起きる——「何も無い」とは言わない */
  checking: boolean;
  /** **返事がないまま見切ったフォルダ。** `checking` と別なのは、あちらが
