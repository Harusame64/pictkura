...
/** id・組の鍵・RAW か。名前は読みやすさのため（関数は見ない） */
const f = (id: number, shot_key: number, is_raw: boolean, name = "") =>
  ({ id, shot_key, is_raw, name }) as Stackable & { name: string };
...
test("重ねの位置は組の最初の1件が居た位置（並びを崩さない）", () => {
  const day = [f(5, 1, false), f(1, 10, false), f(6, 2, false), f(2, 10, true)];
  const stacks = stacksOfDay(day, { rawJpeg: true });
  assert.deepEqual(leads(stacks), [5, 1, 6]);
  assert.deepEqual(members(stacks), [[5], [1, 2], [6]]);
});
...
test("見えているタイルの枚数: 重ねの組は1枚と数える", () => {
  ...
  const index = stackMembersIndex(stacksOfDay(day, { rawJpeg: true }));
  assert.equal(countPhotos([1, 2, 3], index), 2);
  assert.equal(countPhotos([1, 2], index), 1);
  assert.equal(countPhotos([2], index), 1, "組の片方だけでも1枚");
  assert.equal(countPhotos([3], index), 1);
  assert.equal(countPhotos([], index), 0);
  // 重ねを切ったら、ファイルの数そのもの
  const off = stackMembersIndex(stacksOfDay(day, { rawJpeg: false }));
  assert.equal(countPhotos([1, 2, 3], off), 3);
});
