...
pub const RAW_EXTENSIONS: &[&str] = &[
    ...
    // どちらも `絵なし(未確定)`）。**この `arq` はそこに居ない**——325MBで
    // `scannable_whole` は偽になるが、先頭16MBの中にプレビューがあるので
    // その問いに行き着く前に `1616x1080` が出る。足して増えるのは
    // **落ちうる母数**であって、落ちている実例ではない。直すなら
    // 「探し尽くした」とは別に「大きすぎて諦めた」を持たせる話になる
    // （ゲート2・2026-09-04）
