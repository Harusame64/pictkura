...
| **How the capture date is decided** | EXIF → OS properties → **the date already recorded for that photo** → date in the file name → file modification time. Screenshots and saved images without EXIF still land on the right day if their name carries a date. A photo that is looked at again later keeps the date that was read the first time, so a pass that reads nothing cannot move it back to the day it was copied — unless the recorded date is the modification time, which means nothing has been read yet |
