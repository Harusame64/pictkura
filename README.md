...
| **How the capture date is decided** | EXIF → OS properties → **date in the file name** → file modification time. Screenshots and saved images without EXIF still land on the right day if their name carries a date. A file pictkura can make no picture for — a RAW with no usable preview — is looked at again on every startup, and there a date already recorded for it is preferred over the file name and the modification time, so a pass that reads nothing cannot move it back to the day it was copied |
| **A RAW that declares its size nowhere shows none** | Dimensions come from the file's own declaration, or from the sensor borders on Panasonic-built bodies, or from the embedded preview. A RAW with none of those — the Blackmagic CinemaDNG in the sample set — has no size in the grid, and keeps being asked again on every startup |
