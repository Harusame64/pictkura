...
- **サムネイル**はOSから借ります。**Windows** はシェルで、上に並べたコンテナを全部まかないます。
  **macOS** は AVFoundation で、上のコンテナを全部渡したうえで**開けたものだけ**が返ります
  ——`mp4` と `mov` は絵が出ることを、`.avi` は何も返らないことを実測済みです。
  **それ以外は未測定**で、出る可能性はありますが約束はできません。
  **Linux** は経路そのものがありません。
...
| **動画のサムネイルはWindows以外で狭くなる** | Windowsはシェルで、上に並べたコンテナを全部まかないます。**macOS** は AVFoundation が開けたものだけ——`mp4` と `mov` は出る、`.avi` は出ない、を実測済みで、残りは未測定です。**Linux**（各ディストリのサムネイラ）は経路そのものがありません |
