...
<!-- 2回とも取り下げ済み。ただし**また出ることがある**ので消さない。3回目が来たら、
     ...
     日付と版を1行足す -->
**Defender の隔離（2回報告し、2回とも取り下げられました）**

`pictkura.exe` が `Trojan:Win32/Bearfoos.A!ml` として隔離されたことが2回あります——
**0.2.6 が 2026-09-03**（[#105](https://github.com/Harusame64/pictkura/issues/105)）と、
**0.2.7 が 2026-09-06**（[#121](https://github.com/Harusame64/pictkura/issues/121)）です。
末尾の `!ml` は**機械学習による推定**で、中身を特定の脅威と照合した結果ではなく、署名の
無い実行ファイルに出やすい判定です。どちらも Microsoft へ誤検知として報告し、**2回とも
検出が取り下げられました**（2回目は報告した当日）——マルウェアの基準を満たさない、という回答です。

**0.2.7 のダウンロードは 2026-09-06 にいったん取り下げ、同じバイト列のまま戻しました。**
公開している `SHA256SUMS.txt` は最初の公開から変わっていません。ただし**「もう出ない」とは
書けません**——この判定は**クラウドから配られる**もので手元の定義だけでは決まらず、実際に
一度は再発していますし、取り下げが及ぶのは**報告したファイルだけ**です。**issue #121 を
閉じずに開けてあるのはこのため**で、もし隔離されたら**定義の版と出た時刻**をそこへ書いて
いただけると助かります。定義が古いままの機械では、まだ隔離されることがあります。
その場合は**管理者として実行した**コマンドプロンプトで:
