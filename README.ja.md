...
小さくて速い、写真管理のデスクトップアプリです。できることは2つだけ——カメラの
カードから取り込むことと、手元の写真を見ること。数万枚あっても待たされないことを
目標にしています。
...
| **Windows・PC全体へ入れる** | `pictkura_<バージョン>_x64_<言語>.msi` | PCを使う全員にインストールする場合。**管理者権限を求められます**。日本語版と英語版は別のファイルです |
...
> （UAC）を求められます**。
>
> **1台を複数人で使っている場合は注意してください。** MSI版はPCの全員に入るので、
> 乗り換えると**ほかの利用者からは pictkura が消えます**（`-setup.exe` は自分のぶん
> しかインストールしないためです）。使う人それぞれが `-setup.exe` を実行してください。
> また、ほかの利用者が一度でも起動していた場合、**その人の環境には自動再生の候補
> だけが残ります**——pictkura を入れ直して設定の「USBやSDカードを挿したとき」をオフに
> するか、入れ直さないなら Windows の設定で自動再生の既定を選び直してもらってください。
>
...
フォルダ名に日付を入れる場合は、**どの言語でも年から始まる並び**で書き出します。
...
その中だけを移動し、右上の枚数も選んだ数になります。連写の中から1枚を選ぶ作業が、
`P` と `→`（自動送りが入っていれば `P` だけ）で最後まで進められます。
...
起動して数秒たつと、新しい版が出ていないかを1日1回だけ確認します。出ていれば
画面の下に控えめな行が現れ、押すとダウンロードページがブラウザで開きます。
**アプリが勝手に入れ替わることはありません。**

これが**このアプリの唯一の外向き通信**です。送るのは「pictkura のこの版が
問い合わせている」という情報だけで、写真もファイル名もフォルダの場所も送りません。
⚙（設定）→「**このアプリについて**」→「起動時に新しい版を確認する」をオフにすると、
...
pictkura は RAW を**現像しません**。カメラが自分の背面液晶のために書き込んだ
表示用JPEGを取り出して使います。1枚あたり数ミリ秒で済み、色もカメラが出した
ものになります。
...
動画のサムネイルはOSの仕組みから借ります（**現在はWindowsのみ**）。
...
| 事柄 | 内容 |
|---|---|
| **クラウドにしか実体が無いファイル** | OneDrive等の「オンラインのみ」ファイルを、**pictkura から勝手にダウンロードすることはありません**。一覧には出て、寸法と撮影日時はOSが持っているぶんを使います（長さは取れません）。実際に開いたときに初めて実体を取りに行きます |
| **HEIC / HEVC にはOSの部品が要る** | Windowsでは「HEIF Image Extensions」（無料）に加え、画素の展開に「HEVC Video Extensions」（有料・数百円）が要ります。**HEVCのデコーダは同梱しません**——配布には特許の許諾が要るので、OSが持っているものを使う方針です |
| **動画のサムネイルはWindowsのみ** | macOS（QuickLook）・Linux（各ディストリのサムネイラ）は未対応です。枠だけが並びます |
| **`.m2ts` / `.avi` はアプリ内で再生できない** | WebViewのエンジンがこれらのコンテナを扱えないためです。一覧には出ます |
