...
**0.2.7 は Defender に隔離されます（2026-09-06・未解決）**

**0.2.7 のダウンロードは取り下げました。当面は 0.2.6 をお使いください。** 0.2.7 を導入すると、
**インストーラが終わってから約7秒後**に `pictkura.exe` が `Trojan:Win32/Bearfoos.A!ml` として
隔離されます。末尾の `!ml` は**機械学習による推定**で、中身を特定の脅威と照合した結果では
ありません。この判定は**クラウドから配られる**もので静的な定義ではないため、**インストーラも、
その中身も、スキャンでは何も出ません**——公開した時点では白で、あとから黒くなったのは
このためです。**2026-09-06** に Microsoft へ誤検知として報告しました。同じ検出は過去2回
（2026-09-03 と 2026-09-04）、いずれも1日以内に取り下げられています。

**すでに隔離された方へ。写真とライブラリは無傷です。** 掃除されるのは
`%LOCALAPPDATA%\pictkura\` で、データベースと設定は `%APPDATA%\` の下です。
**0.2.6 を上から入れれば戻ります。** 定義が古いままの機械でまだ隔離される場合は、
**管理者として実行した**コマンドプロンプトで:
...
経緯は [issue #121](https://github.com/Harusame64/pictkura/issues/121) にまとめてあります（0.2.6 のときの一件は [#105](https://github.com/Harusame64/pictkura/issues/105)）。
...
**1,680枚から絵が出て、うち1,495枚が原寸**でした。ただし `ori` と `arq` は 0.2.6 のタグを
切った後に足したものなので、**公開中の 0.2.6 が探索するのは1,831枚のうち1,811枚**——
絵が出るのは1,660枚、うち原寸は1,475枚です。`rw2` `cr3` `pef` `srw` `rwl` `x3f`
...
**公開中の 0.2.6 は `.ori` を探索しない**ので、あちらでは同じ1回が2本のままです。
