...
| **撮影日時の決め方** | EXIF → OSのプロパティ → **その写真について既に記録してある日付** → ファイル名の日付 → ファイルの更新日時、の順です。EXIFを持たない保存画像（スクリーンショット等）でも、名前に日時があれば正しい日に並びます。あとから調べ直される写真は、**1回目に読めた日付を保ちます**ので、何も読めなかった回に取り込んだ日へ戻ることはありません——記録してあるのが更新日時そのもののとき（まだ何も読めていない印）を除きます |
