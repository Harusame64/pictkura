...
| 対象 | ファイル | 備考 |
|---|---|---|
| **Windows 10/11（x64）・おすすめ** | `pictkura_<版>_x64-setup.exe` | **管理者権限は要りません**。自分のユーザーだけに入ります（`%LOCALAPPDATA%\pictkura`）。日本語と英語は最初に選べます |
| **Windows・PC全体へ入れる** | `pictkura_<版>_x64_<言語>.msi` | PCを使う全員に入れる場合。**管理者権限を求められます**。日本語版と英語版で別ファイルです |
| **Windows・インストール不要** | `pictkura_<版>_x64-portable.zip` | 展開して `pictkura.exe` を実行するだけ |
...
> `-setup.exe` はMSI版が入っていることに気づくと、**先にMSI版を消してから**入れます。
> ただしMSIを消すのはPC全体に触る操作なので、**そこだけ管理者の許可（UAC）を聞かれます**。
>
> **1台を何人かで使っている場合は注意してください。** MSI版はPCの全員に入っています。
> 乗り換えると**ほかの人からは pictkura が消えます**（`-setup.exe` は自分のぶんだけ
> 入れるため）。使う人それぞれが `-setup.exe` を入れてください。また、ほかの人が
> 一度でも起動していた場合、**その人の自動再生の候補だけが残ります**——その人が
> pictkura を入れ直して設定の「USBやSDカードを挿したとき」を切るか、入れ直さない
> なら Windows の設定で自動再生の既定を選び直してください。
>
> 逆向き（`-setup.exe` を入れた状態でMSIを入れる）は面倒を見てくれないので、
> そのときは先に「設定 → アプリ」から消してください。

### Windows は署名していないので警告が出ます

pictkura は **Windows のコード署名を受けていません**（有料の証明書が要るため）。
壊れてはいませんが、下記のセキュリティ警告が出ます。進めば普通に使えます。
**SmartScreen はダウンロードした版ごとに出ることがあります**。

**`-setup.exe`（おすすめ）でインストールするとき**

出るのは **SmartScreen だけ**です（「**WindowsによってPCが保護されました**」→
「**詳細情報**」→「**実行**」）。**UAC は出ません**——PC全体ではなく
自分のユーザーの中にだけ入れるので、管理者の許可が要らないためです。

**例外が2つあります**。どちらも「PC全体に触る作業を代わりにやる」ときです:

1. 以前の**MSI版が入っている場合**、`-setup.exe` はそれを先に消してから入れます。
   **その1回だけ UAC が出ます**。次回からは出ません
2. **WebView2 ランタイムが入っていないPC**では、インストーラがそれを取ってきて
   入れようとします。WebView2 はPC全体に入るものなので、ここでも UAC が出ます
   （Windows 11 と更新済みの Windows 10 には最初から入っているので、普通は出ません）
...
あります。これは**どの入手方法でも同じ**です（届き方ではなくコード自体を見るため）。
...
> 必ず同じ日のフォルダへ入ります（片方だけ撮影日時を持たないことがあるため）。
...
| `X` | ボツの候補にする（✕）。押した時点では**何も消えません**——閉じるときに顔を並べて確かめてから、まとめてゴミ箱へ |
...
タイルにマウスを載せると角に丸が出ます。押すか、`Ctrl` を押しながらタイルをクリック
（macOS は `⌘`）すると選択が始まり、画面の上に選択バーが出ます。
...
選択中は、ふつうのクリックがビューアではなく選択の入れ替えになります。バーからは
選択したぶんの ★ の付け外し、選んだフォルダへのコピー・移動、まとめてゴミ箱へ移動が
できます。削除は確認してから実行し、**削除は変わらずゴミ箱経由**です。

**選んだぶんを見る**を押すと、選んだ写真だけをビューアで順に送れます。`←` `→` は
その中だけを歩き、右上の枚数も選んだ数になります。連写の中から1枚を選ぶときに、
`P` と `→`（自動送りが入っていれば `P` だけ）で歩き切れます。

⚑ の印は **★ お気に入りとは別の棚**です。★は「あとで見返したい写真」、⚑は
「この連写から残す1枚」——混ぜると、選別のたびにお気に入りが選別の残りで
埋まってしまうためです。左の **⚑ 選別で選んだもの** から選んだぶんだけを
一覧でき、検索ボックスでは `⚑`（または `pick:`）でも絞れます。

**フォルダへコピー／フォルダへ移動**は、選んだファイルを指定したフォルダへ
**そのまま平置き**します（日付のフォルダは作りません）。何枚か人に渡す・USBメモリへ
入れる、といった用途向けです。同じものが既にあれば何もしません
（名前・サイズ・更新日時で見ます）。同じフォルダへ何度書き出しても増えません。
名前が同じでも中身が違うときは `-1`, `-2` を付けて避けます。移動は確認してから実行し、
**その写真は元の場所から無くなり、ライブラリからも外れます**（★と⚑の印は
引き継がれません）。別のドライブへ移すとき、および
クラウドにしか実体が無いファイルを移すときは、コピーしてから元をゴミ箱へ送ります。
...
OSの優先言語に従い、対応が無ければ英語になります。⚙（設定）→「**言語**」で
明示的に選べます（切り替えると画面を読み込み直します）。

取扱説明書はアプリに同梱していて ⚙（設定）→「**取扱説明書**」から開けます。
...
起動して数秒たつと、新しい版が出ていないかを1日1回だけ聞きに行きます。出ていれば
画面の下に控えめに出て、押すとダウンロードページがブラウザで開きます。
**アプリが勝手に入れ替わることはありません。**

これが**このアプリの唯一の外向き通信**です。送るのは「pictkura の版いくつが
聞いている」という名乗りだけで、写真もファイル名もフォルダの場所も送りません。
⚙（設定）→「**このアプリについて**」→「起動時に新しい版を確認する」を切ると、
pictkura は一切外へ出ません（同じ場所の「更新を確認」を押したときだけ聞きに行きます）。
