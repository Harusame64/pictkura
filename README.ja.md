...
壊れてはいませんが、インストールと初回起動でセキュリティの警告が出ます。**いずれも
初回だけ**で、進めば普通に使えます。

**MSI でインストールするとき**

1. **SmartScreen**（ダウンロード直後の初回だけ）: 「**WindowsによってPCが保護されました**」と
   出ることがあります。「**詳細情報**」を押すと「**実行**」ボタンが現れるので、それで進みます。
   一度実行すると次からは出ません（**同じ MSI を入れ直しても再現しないことがあります**）。
2. **ユーザーアカウント制御（UAC）**: 続けて「このアプリがデバイスに変更を加えることを
   許可しますか？」と聞かれ、**発行元が「不明」**と黄色く出ます（署名が無いため）。
   「**はい**」で進みます。こちらは署名するまで毎回出ます。
...
> 署名（Authenticode）をすればこれらの警告は消えますが、証明書が有料のため
> **v0.1 では見送っています**。macOS 版も同じく署名していません（下記）。
