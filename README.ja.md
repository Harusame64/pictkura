...
<!-- 3回とも取り下げ済み。ただし**また出ることがある**ので消さない。4回目が来たら、
     ...
     日付と版を1行足す。**4か所そろえること** -->
**Defender の隔離（3回とも取り下げられました）**

pictkura の実行ファイルが `Trojan:Win32/Bearfoos.A!ml` として隔離されたことが**3回**あります。
うち配っていた版は2回で、**0.2.6 が 2026-09-02**（翌 09-03 に取り下げ・[#105](https://github.com/Harusame64/pictkura/issues/105)）と、
**0.2.7 が 2026-09-06**（同日に取り下げ・[#121](https://github.com/Harusame64/pictkura/issues/121)）です。
残る1回（2026-09-04）は**公開前に試していた版**で、誰にも配っていません。
末尾の `!ml` は**機械学習による推定**で、中身を特定の脅威と照合した結果ではなく、署名の
無い実行ファイルに出やすい判定です。**3回とも Microsoft へ誤検知として報告し、3回とも
検出が取り下げられました**——マルウェアの基準を満たさない、という回答です。
...
**3回起きて3回消えている**以上また起きうるものですし、取り下げが及ぶのは**報告した
ファイルだけ**です。**issue #121 を閉じずに開けてあるのはこのため**で、もし隔離されたら
