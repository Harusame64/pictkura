...
上は**コマンドプロンプト**（`cmd.exe`）の書き方で、つまずきどころが2つあります。
**別のドライブに居るときは `cd` でドライブを跨げません**——`cd /d` と書いてください。
PowerShell の場合は現在のディレクトリを探しに行かないので、2行目と3行目を
`.\MpCmdRun.exe` と書きます。
...
**画像が出ること・縦位置が縦で出ること・撮影日時とカメラ名が読めること**を確かめています。
