...
**上の表の一覧欄の ✅ は、Windowsでの話です。** その裏にある2つは、他のOSでは狭くなります。

- **サムネイル**はOSから借ります。**Windows** はシェルで、上に並べたコンテナを全部まかないます。
  **macOS** は AVFoundation で、QuickTime / MPEG-4 系しか開かないため、絵が出るのは
  `mp4` `m4v` `mov` `3gp` だけです——**それ以外は `webm` も含めて枠のまま**になります。
  **Linux** は経路そのものがありません。
  （macOSでの実測: `mp4` と `mov` は絵が出る、`.avi` は何も返らない）
- **長さ・寸法・撮影日時**は、`mp4` `m4v` `mov` `3gp` の4つだけコンテナのヘッダから読みます
  （画素は1バイトも読みません）。それ以外は **Windows** ではシェルが埋めますが、
  **macOS** と **Linux** では埋まらないので、長さも寸法も出ず、日付はファイル名か
  更新日時に落ちます。
...
| **動画のサムネイルはWindows以外で狭くなる** | Windowsはシェルで、上に並べたコンテナを全部まかないます。**macOS** は AVFoundation で、`mp4` `m4v` `mov` `3gp` しか開かないため、それ以外は枠のままです。**Linux**（各ディストリのサムネイラ）は経路そのものがありません |
| **`mp4` `m4v` `mov` `3gp` 以外の長さ・寸法** | この4つはコンテナのヘッダから直接読みます。それ以外は **Windows** のシェル頼みで、**macOS**・**Linux** では長さも寸法も出ず、日付はファイル名か更新日時に落ちます |
