...
| 対象 | ファイル | 備考 |
|---|---|---|
| **Windows 10/11（x64）・おすすめ** | `pictkura_<バージョン>_x64-setup.exe` | **管理者権限は不要です**。自分のユーザーにだけインストールされます（`%LOCALAPPDATA%\pictkura`）。**インストーラ**の言語は日本語か英語を選べます（アプリは OS の言語か、設定で選んだ言語で表示されます） |
...
> また、ほかの利用者の環境に**自動再生の候補だけが残っていることがあります**
> （0.2.7 までは、起動しただけで登録されていました）。その人に、pictkura を入れ直して
> 設定の「USBやSDカードを挿したとき」をオフにしてもらうか、入れ直さないなら
> Windows の設定で自動再生の既定を選び直してもらってください。
>
...
<!-- 配った版の2回は取り下げ済み。ただし**また出ることがある**ので消さない。次が来たら、
     この段落・README.md の同じ段落・docs/{ja,en}/install.html の 2.5 節と目次の行に
     日付と版を1行足す。**4か所そろえること** -->
**Defender の隔離（配った版は2回。どちらも取り下げ済み）**

pictkura の実行ファイルが `Trojan:Win32/Bearfoos.A!ml` として隔離されたことが、これまでに
**3つのビルド**であります——**0.2.6**（2026-09-02）、**公開前に試していた 0.2.7**（09-03〜04）、
**公開した 0.2.7**（09-06）。末尾の `!ml` は**機械学習による推定**で、中身を特定の脅威と
照合した結果ではなく、署名の無い実行ファイルに出やすい判定です。

**配っていた版は2回で、どちらも Microsoft へ誤検知として報告し、検出は取り下げられました**
——マルウェアの基準を満たさない、という回答です。**0.2.6 が 2026-09-02**（翌 09-03 に
取り下げ・[#105](https://github.com/Harusame64/pictkura/issues/105)）と、**0.2.7 が
2026-09-06**（同日に取り下げ・[#121](https://github.com/Harusame64/pictkura/issues/121)）です。
**0.2.6 のときは `-setup.exe` のほうも `Bearfoos.B!ml` として隔離されましたが、提出したのは
実行ファイルだけ**——ここでも、取り下げが及んだのは報告した1本です。

**残る1つは公開前の 0.2.7 で、誰にも配っていません**（そのとき配っていたのは 0.2.6 の
ままでした）。ここでは**同じビルドから出た2つのファイルが、別々に隔離されました**——そして
**報告して取り下げられたのは、そのうち片方だけ**です。もう片方は、取り下げが届いたあとも
**また隔離されました**。**取り下げはファイル1本ごとにしか効かない**、というのがここで
分かったことです。

**すでに隔離された方へ。写真とライブラリは無傷です。** 掃除されるのは
`%LOCALAPPDATA%\pictkura\` で、データベースと設定は `%APPDATA%\` の下——別の場所です。
実体を戻す手順は下のコマンドの後に書いてあります——**定義を新しくしてから**入れ直します。

**0.2.7 のダウンロードは 2026-09-06 にいったん取り下げ、同じバイト列のまま戻しました。**
公開している `SHA256SUMS.txt` は最初の公開から変わっていません。ただし**「もう出ない」とは
書けません**——この判定は**クラウドから配られる**もので手元の定義だけでは決まらず、
**配った版で2回、公開前の試験でも起きている**以上また起きうるものですし、取り下げが
及ぶのは**報告したファイル1本だけ**です——0.2.7 では**インストーラの中身**で、
**MSI と持ち歩き版はそれぞれ別のバイト列**（どちらも隔離されたことはありません）。**issue #121 は閉じましたが、経緯はそのまま残してあります**——もし隔離されたら
**定義の版と出た時刻**をそこへ書いていただけると助かります（**再発したら開け直します**）。定義が古いままの機械では、
まだ隔離されることがあります。その場合は**管理者として実行した**コマンドプロンプトで:
...
| `★` | お気に入りだけ |
| `⚑` / `pick:` | 選別で選んだものだけ |
| `kind:raw` / `種類:動画` | 種類で絞り込む（`画像` ／ `raw` ／ `動画`） |
...
| `Ctrl` + `C` / `Ctrl` + `S` | いま出ている絵をクリップボードへ／ファイルに保存（macOSは `⌘`） |
| `1` / `0` | `1` は等倍100%と「画面に合わせる」の**切り替え**、`0` は必ず画面に合わせる |
| `F11` | 全画面 |
| `Esc` | 閉じる |
| `?` | ショートカット一覧を出す（一覧でもビューアでも。ダイアログが出ているときは開きません） |
| `Ctrl` + `,` | 設定を開く（macOSは `⌘` + `,`、またはメニューの「pictkura → 設定…」） |

マウスを止めると操作パネルが自動で消え、動かすと戻ります。
右クリックで「開く／他のアプリで開く…／ファイルの場所を開く／お気に入りに追加／選ぶ／削除（ゴミ箱へ）」（一度使った編集アプリは「開く」のすぐ下に並びます）。
...
引き継がれません）。移動先に同じものが既にあった写真と、写せなかった写真は、元の場所と
ライブラリに残ります。別のドライブへ移すとき、およびクラウドにしか実体が無いファイルを
...
画面は **英語・ドイツ語・スペイン語（スペイン）・スペイン語（中南米）・中国語（簡体字）・
中国語（繁体字）・日本語** で使えます。

OSの優先言語に従います。対応する言語が無い場合は英語です。⚙（設定）→「**言語**」で
明示的に選べます（切り替えると画面を読み込み直します）。週の始まりは OS の設定に
従います。保存やフォルダ選びなどのシステムのダイアログの枠は OS のもので、OS の言語で
出ます（macOS では、macOS の言語が上の7つに無ければ英語です）。
...
——メーカー56社・923機種を、測った実物へのリンクつきで並べてあります。

先に知っておくとよいことが1つ。**ハイレゾ撮影は、タイル3つになります。**
OM System の機種は1回のシャッターで `.ORF`・`.ORI`・`.JPG` の**3本をカードに書く**ので、
`.ori` を足したことで**3本とも一覧に並びます**。**カードに在るファイルを隠すほうが悪い**、という判断です。
...
気付けません。`cargo tauri build` には Tauri CLI が要ります。**版はCIと同じものに
固定してください**（`cargo install tauri-cli --version 2.11.4 --locked`）。

Windows では、この「バンドル」を2段に割って、あいだで1つだけ手を入れています。
バンドラは**入れ物ごとに実行ファイルの中の27バイトの目印を書き換える**ので、
放っておくと**1回のビルドから3バイト違いのバイト列が3本**出ます。ウイルス対策の
判定は**バイト列1本ごとに付く**ため、これは「くじが3本」ということで、0.2.7 では
**取り下げた1本が、利用者の走らせない側**でした。そこで `--no-bundle` で本体だけ作り、
**目印を潰してから**束ねます——バンドラは目印を見つけられず、警告を出して、
**3つの入れ物すべてに同じ実行ファイル**を入れます。リリースの工程では、
**作った物から実行ファイルを取り出し直してハッシュが一致するか**を確かめ、
違えば落とします——NSISのインストーラ・MSI 2本・持ち歩き版の**4つとも**で、
**どれか1つでも取り出せなければ、そこでも落とします**（測れていない入れ物を
「一致」に数えないため）。
...
（**失敗したときだけ**作られ、送信はしません。設定 →「このアプリについて」→
