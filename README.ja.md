...
ダブルクリックしても起動せず、こう表示されます。壊れてはいません——未署名のアプリに
対してGatekeeperが出す定型の文言です。

> **"pictkura" は開いていません**
> Apple は、"pictkura" に Mac に損害を与えたり、プライバシーを侵害する可能性のある
> マルウェアが含まれていないことを検証できませんでした。
> ［ゴミ箱に入れる］［完了］

このダイアログには先へ進むボタンがありません。**［完了］を押してから**、次の手順で開きます。

1. `pictkura.app` を「アプリケーション」フォルダなど、**置いておきたい場所へ移す**
2. ダブルクリックする（上のメッセージが出るので［**完了**］）
3. **システム設定** →「**プライバシーとセキュリティ**」を開き、**下の方までスクロール**する
...
> **右クリック（control＋クリック）→「開く」は効きません。** macOS 14 まではこれで
> 開けましたが、**macOS 15 以降この抜け道は廃止されました**。古い手順を書いた記事が
> まだ多く残っているので注意してください。

手順1で先に移しておくのは作法ではありません。移さずに起動すると、macOSはアプリを
