...
3回あります。末尾の `!ml` は**機械学習による推定**で、中身を特定の脅威と照合した結果では
なく、署名の無い実行ファイルに出やすい判定です。
...
残る1回は **2026-09-03** で、**公開前に試していた 0.2.7** です。**誰にも配っていません**
（そのとき配っていたのは 0.2.6 のままでした）。**この回は、隔離されたファイルそのものの
検出は取り下げられていません**——報告して取り下げられたのは、**同じビルドから出た別の
ファイル**でした。**取り下げはファイル1本ごとに効く**、というのがこの回で分かったことです。

**すでに隔離された方へ。写真とライブラリは無傷です。** 掃除されるのは
`%LOCALAPPDATA%\pictkura\` で、データベースと設定は `%APPDATA%\` の下——別の場所です。
**定義を新しくしてから**入れ直せば実体が戻ります（コマンドは下）。定義が古いままだと、
入れ直したものがまた持っていかれます。
...
及ぶのは**報告したファイル1本だけ**です。**issue #121 を閉じずに開けてあるのはこのため**で、もし隔離されたら
