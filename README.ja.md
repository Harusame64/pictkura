...
| 対象 | ファイル | 備考 |
|---|---|---|
| **Windows 10/11（x64）・おすすめ** | `pictkura_<バージョン>_x64-setup.exe` | **管理者権限は不要です**。自分のユーザーにだけインストールされます（`%LOCALAPPDATA%\pictkura`）。**インストーラ**の言語は日本語か英語を選べます（アプリは OS の言語に従います） |
...
明示的に選べます（切り替えると画面を読み込み直します）。日付・数字・週の始まりは、
表示言語によらず OS の地域の設定に従います。macOS では、保存やフォルダ選びなどの
システムのダイアログの枠が macOS の言語で出ます。
