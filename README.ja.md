...
| **撮影日時の決め方** | EXIF → OSのプロパティ → **ファイル名の日付** → ファイルの更新日時、の順です。EXIFを持たない保存画像（スクリーンショット等）でも、名前に日時があれば正しい日に並びます。**絵を作れないファイル**（使えるプレビューを持たないRAW等）は起動のたびに調べ直されるので、そこでは**既に記録してある日付**をファイル名や更新日時より先に見ます——何も読めなかった回に、取り込んだ日へ戻らないようにするためです |
