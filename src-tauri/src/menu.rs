...
struct MenuText {
    about: &'static str,
    /// **設定への2本目の道**（2026-09-07）。**画面の歯車は、狭いツールバーから
    /// 落ちることがある**——**macOS の独語で 1072px 未満**（2026-09-07 実測）。
    /// **Windows では同じ幅で落ちない**（2026-09-08・win が全7言語で実測。
    /// 切れ始めるのは 620 付近。**差は字体と見ているが、測っていない**）。
    /// **設定への入口はあの1つだけ**だったので、**幅に依らない道が要る**
    /// ——この理由は、歯車が見えている台でも成り立つ。
    settings: &'static str,
    services: &'static str,
    hide: &'static str,
        ...
        &[
            &Submenu::with_items(
                ...
                &[
                    ...
                    // **設定への2本目の道。** **設定への入口は歯車1つだけ**だったので、
                    // **窓の幅に依らない道を1本足す**。
                    //
                    // **歯車が落ちる幅は台で違う**——**macOS の独語では 1072px 未満**で
                    // 押し出される（2026-09-07 実測。960 でも 1024 でも消える）が、
                    // **Windows では 960 で溢れない**（2026-09-08・win の実測）。
                    // **「幅で消えるから要る」ではなく「入口が1つしか無いから要る」**
                    // ——後者はどちらの台でも成り立つ。
                    //
